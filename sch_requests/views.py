...
class AvailableUserView(APIView):
    def post(self, request):
        serializer = FindUserRequestSerializer(data=request.data, context = {'request':request})
        ...
        return Response({'message': '요청 데이터가 유효하지 않습니다'}, status=status.HTTP_400_BAD_REQUEST)


class FamScheduleRegisterView(APIView):
    def post(self, request):
        serializer = RegisterScheduleSerializer(data=request.data, context = {'request':request})
        if serializer.is_valid():
            data = serializer.validated_data
